<?php
/**
 * Canal Mautic para Orders2WhatsApp (WooCommerce)
...
class Orders2WhatsApp_Channel_Mautic implements Orders2WhatsApp_Channel_Interface {
    ...
    public function send(array $payload, array $ctx = []): void {
        ...
        // 1) order_id desde el contexto
        $order_id = (int)($ctx['vars']['order_id'] ?? 0);
        if (!$order_id) { $this->step('sin order_id en ctx. Abort.'); return; }

        // 2) Construir campos ricos
...
/** Remaps Woo payload fields to REAL Mautic aliases and strictly filters to a whitelist.
 *  - Ensures/normalizes last_order_status (strips "wc-", lowercases, maps common variants).
 *  - Accepts status aliases: last_order_status | status | order_status | wc_status | last_status
 *  - Falls back to last_order_json.status if present.
 *  - last_ord_products stored as "name | url | image" lines (no JSON).
 *  - last_purchase_date normalized to 'Y-m-d H:i:s' (UTC).
 */
private function remap_and_filter_fields(array $f): array {
    // Safe unicode length/substring (works even if mbstring is missing)
    ...
    // ❌ Fields we will NOT send directly
    $deny = [
        'last_order_currency', // not in Mautic
        'last_order_json',     // moved to Note
        // sources for fallbacks only:
        // 'last_order_categories', 'last_order_items_json'
    ];

    // ✅ Map (payload) -> REAL Mautic aliases
    $map = [
        'firstname'              => 'firstname',
        'lastname'               => 'lastname',
        'email'                  => 'email',
        ...
        'last_order_id'          => 'last_order_id',        // text
        'last_order_amount'      => 'last_order_amount',    // number

        'historic_purch_amount'  => 'historic_purch_amoun', // number (Mautic alias)
        'historic_purch_amoun'   => 'historic_purch_amoun', // number
        'historic_purch_count'   => 'historic_purch_event', // number
        'historic_purch_event'   => 'historic_purch_event', // number

        // Status aliases → all land in last_order_status
        ...
        'last_purchase_date'     => 'last_purchase_date',   // datetime
    ...
    $normalize_status = function (string $raw) use ($u_strlen, $u_substr): string {
        ...
        $v = isset($mapStatus[$s]) ? $mapStatus[$s] : $s; // keep custom statuses
    ...
    // Convert array/json items to lines "name | url_product | url_image"
    ...
    // Fallback for status from last_order_json.status
    ...
    /** Crea una nota para el contacto.
     *  Intenta 3 variantes de payload (algunos Mautic aceptan una y otros otra):
     *   1) JSON con wrapper { "note": { ... } }
     *   2) JSON plano { "text": "...", "type": "...", "lead": 123 }
     *   3) x-www-form-urlencoded: note[text]=...&note[type]=...&note[lead]=...
     */
    private function create_note(string $base, string $mode, string $tokenFieldRaw, int $contactId, string $text, array $ctx = []): bool {
        ...
        // --- intento 1: JSON con wrapper "note"
        ...
        // --- intento 2: JSON plano sin wrapper
        ...
        // --- intento 3: application/x-www-form-urlencoded con note[...]
    ...
    private function maybe_log_raw(string $url, array $headers, string $body, int $http_code, ?string $resp, ?string $curl_err, array $payload, array $ctx): void {
        ...
        $dir    = trailingslashit($upload['basedir']) . '7c-wc-orders2whatsapp';
        if (!file_exists($dir)) {
            wp_mkdir_p($dir);
            $this->step('creando dir de logs: ' . $dir);
        }

        $order_id  = $ctx['vars']['order_id'] ?? ($payload['json']['order']['id'] ?? 'unknown');
        $stamp     = current_time('Ymd_His');
        $filename  = sprintf('mautic-%s-%s.txt', $order_id, $stamp);
