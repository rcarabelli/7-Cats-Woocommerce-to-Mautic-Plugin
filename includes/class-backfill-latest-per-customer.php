...
class Orders2WhatsApp_Backfill {
    ...
    public static function reset(): void {
        ...
        delete_option(self::OPT_DONE_SET);
    ...
    public static function cron_worker(int $batch_size = 25): void {
        ...
        set_transient(self::TR_LOCK, 1, 60);
...
function o2w_backfill_redirect_back() {
    $back = wp_get_referer();
    if (!$back) {
        $back = admin_url('admin.php?page=orders2whatsapp_mautic_sync');
    }
    return $back;
}

add_action('admin_post_orders2whatsapp_backfill_run_one', function () {
    if (!current_user_can(S2M_REQUIRED_CAP)) wp_die('Forbidden', 403);
    check_admin_referer('o2w_backfill_run_one');
    Orders2WhatsApp_Backfill::run_one();
    wp_safe_redirect(o2w_backfill_redirect_back());
    exit;
});
...
add_action('admin_post_orders2whatsapp_backfill_reset', function () {
    if (!current_user_can(S2M_REQUIRED_CAP)) wp_die('Forbidden', 403);
    check_admin_referer('o2w_backfill_reset');
    Orders2WhatsApp_Backfill::reset();
    wp_safe_redirect(o2w_backfill_redirect_back());
    exit;
});

add_action('admin_post_orders2whatsapp_backfill_build', function () {
    if (!current_user_can(S2M_REQUIRED_CAP)) wp_die('Forbidden', 403);
    check_admin_referer('o2w_backfill_build');
    $built = Orders2WhatsApp_Backfill::build_queue();
    $count = is_array($built['queue'] ?? null) ? count($built['queue']) : 0;
    set_transient(Orders2WhatsApp_Backfill::TR_MSG, "Queue built: {$count} items queued.", 30);
    wp_safe_redirect(o2w_backfill_redirect_back());
    exit;
});
